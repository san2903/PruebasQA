...
import lib.helpers.generalhelpers
...
def step_impl(context, assertion, page_name):
    validation_result = lib.helpers.generalhelpers.validate_text(assertion, page_name, context.current_page.get_title_page())
...
def step_impl(context, expression):
    list_validation = context.driver.are_element_presents(context.table, context)
    assertion = lib.helpers.generalhelpers.transform_validation(expression)
...
def step_impl(context, element_name, element_type, text_to_validate):
    element = lib.helpers.generalhelpers.transformation_helper(element_name, element_type)
    text_element = GeneralComponents.get_text_element(context, element).rstrip()
    new_text_to_validate = lib.helpers.generalhelpers.join_words(lib.helpers.generalhelpers.split_and_replace_string(text_to_validate))
    new_text_element = lib.helpers.generalhelpers.join_words(lib.helpers.generalhelpers.split_and_replace_string(text_element))
...
def step_impl(context, url):
    GeneralComponents.wait_until_url_is(context.driver, url)
...
def step_impl(context, element_name, element_type, expression):
    element_name = lib.helpers.generalhelpers.transformation_helper(element_name, element_type)
    assertion = lib.helpers.generalhelpers.transform_validation(expression)
